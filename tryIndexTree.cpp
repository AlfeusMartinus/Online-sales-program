...
// TREE UNTUK MEMBUAT SIMPUL BARU
treeNode* buatNode(transaksi data){
...
// TREE UNTUK MEMASUKAN SIMPUL BARU
treeNode* insertNode(treeNode* node, transaksi data){
    if (node == NULL)
    {
    ...
    if (data.noTransaksi < node->data.noTransaksi)
    {
        node->left = insertNode(node->left, data);
    }
    else if (data.noTransaksi > node->data.noTransaksi)
    {
...
// TREE MENAMPILKAN DATA TRANSAKSI
void displayTransaksi(treeNode* node){
    if (node != NULL)
    {
        displayTransaksi(node->left);
        cout << "Nomor Transaksi\t  : " << node->data.noTransaksi << endl
             << "Produk\t\t  : " << node->data.namaProduk << endl
             << "Jumlah\t\t  : " << node->data.Qty << endl
             << "Total Harga\t  : " << node->data.totalHarga << endl
             << "Status Pembayaran : " << node->data.statusPembayaran << endl
             << endl;
        displayTransaksi(node->right);
    }
};

// TREE UNTUK MENCARI TRANSAKSI
treeNode* searchTransaksi(treeNode* node, string noTransaksi){
    if (node == NULL || node->data.noTransaksi == noTransaksi)
    {
        return node;
    }

    if (noTransaksi < node->data.noTransaksi)
    {
        return searchTransaksi(node->left, noTransaksi);
    }
    else {
        return searchTransaksi(node->right, noTransaksi);
    }
};

// TREE UNTUK MENGHAPUS TRANSAKSI
treeNode* deleteTransaksi(treeNode* root, string noTransaksi){
    if (root == NULL)
    {
        return root;
    }

    if (noTransaksi < root->data.noTransaksi)
    {
        root->left = deleteTransaksi(root->left, noTransaksi);
    }
    else if (noTransaksi > root->data.noTransaksi) 
    {
        root->right = deleteTransaksi(root->right, noTransaksi);
    }
    else {
        // Kasus jika simpul yang akan dihapus tidak memiliki anak atau memiliki anak tunggal
        if (root->left == NULL)
        {
        ...
        root->data = temp->data;
        root->right = deleteTransaksi(root->right, temp->data.noTransaksi);
    }
